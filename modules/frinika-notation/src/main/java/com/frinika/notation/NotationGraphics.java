...
public class NotationGraphics {
	...
	{
		if(clef == CLEF_F) drawClef(clef, 6);
		else if(clef == CLEF_C) drawClef(clef, 4);
		else if(clef == CLEF_G) drawClef(clef, 2);
		else if(clef == CLEF_NEUTRAL) drawClef(clef, 4);
		else drawClef(clef, 4);
	}
	...
	{							
		if(n.linedir == 0)
		{
			if(n.note > 4) n.linedir = 2; else n.linedir = 1;
		}
		else
		{
			if(n.linedir == -1) n.linedir = 0;
		}				
		if(n.dur < 1) n.linedir = 0;
		...
		{	
			...
			if(n.linedir != 0 && notepart.linedir != 0)
			{
				if(notepart.linedir != n.linedir) 
				{
					...
					{
						if(notepart2.note < min) min = notepart2.note;
						if(notepart2.note > max) max = notepart2.note;
					}
		...
		if(breakgroup) drawNotes();	
	
		addNoteToGroup(n);
		if(!in_note_group) drawNotes();
	...
	{
		if(dur == -2) return (char)0xE1BD;
		else if(dur == -1) return (char)0xE11A;
		else if(dur == 0) return (char)0xE11B;
		else if(dur == 1) return (char)0xE11C;
		else return (char)0xE11D;				
	}
	...
	{
		if(note_group_list.size() == 0) return;
		...
		for (int i = 0; i < note_group_list.size(); i++) {
			...
			if(notepart.dur > maxdur) maxdur = notepart.dur; 
		}
		if(maxdur > 4)
		{
		...
		{			
			...
			for (int i = 0; i < note_group_list.size(); i++) {
				Note notepart = note_group_list.get(i);
				if(notepart.note > maxnote) maxnote = notepart.note;
				if(notepart.note < minnote) minnote = notepart.note;
			...
			{				
				...
				if(linedir == 1)
				{
					...
					if(dur == 3) g.drawString("" + (char)0xE17F, cx+notebasewidth+(grid*0.1f), cy-(stem_length)-(maxnote*grid*0.5f));
					if(dur == 4) g.drawString("" + (char)0xE180, cx+notebasewidth+(grid*0.1f), cy-(stem_length)-(maxnote*grid*0.5f));
					if(dur == 5) g.drawString("" + (char)0xE181, cx+notebasewidth+(grid*0.1f), cy-(stem_length)-(maxnote*grid*0.5f));
					if(dur == 6) g.drawString("" + (char)0xE182, cx+notebasewidth+(grid*0.1f), cy-(stem_length)-(maxnote*grid*0.5f));
				...
				if(linedir == 2)
				{
					...
					if(dur == 3) g.drawString("" + (char)0xE183, cx+(grid*0.1f), cy+(stem_length)-(minnote*grid*0.5f));
					if(dur == 4) g.drawString("" + (char)0xE186, cx+(grid*0.1f), cy+(stem_length)-(minnote*grid*0.5f));
					if(dur == 5) g.drawString("" + (char)0xE187, cx+(grid*0.1f), cy+(stem_length)-(minnote*grid*0.5f));
					if(dur == 6) g.drawString("" + (char)0xE188, cx+(grid*0.1f), cy+(stem_length)-(minnote*grid*0.5f));
				...
				
		{
			...
			if(linedir == 1)
			{
				...
				for (int i = 0; i < note_group_list.size(); i++) 			
				{
			 	...
			 	if(maxnote_x != maxx && maxnote_x != minx) h = 0;
			...
			if(linedir == 2)
			{
				...
				if(minnote_x != maxx && minnote_x != minx) h = 0;
