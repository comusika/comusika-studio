 ...
 * ProjectContainer. Although the execution of scripts is handled globally, each
 * script runs inside it's associated project's focus, and the set of scripts
 * which are currently loaded and thus available for execution is
 * project-specific.
 *
 * The architecture allows for deploying multiple scripting languages, but the
 * only language used for scripting now is JavaScript, and there are no plans to
 * add support for any other languages. The actual interfacing between the
 * 'Frinika world' and the 'JavaScript world' is done by class
...
public class FrinikaScriptingEngine implements ScriptContainer, Serializable {
    ...
    /**
     * Adds a ScriptListener. This method is static, as scripts can potentially
     * affect the whole running system, across projects, so it doesn't make
     * sense to restrict references to scripts to individual projects.
     *
     * @param l
     */
     ...
     *
     * @param script
     * @param returnValue if ==this, then 'script has started', else 'script as
     * exited'
     */
    ...
    public static void saveString(String s, File file) throws IOException {
        try ( // TODO move to global utilities-class
