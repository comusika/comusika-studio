...
/**
 * Menu-action for inserting controller-events accordig to different, selectable
 * kinds of functions. In a special mode, also note-events can be inserted, or
 * velocity-values of existing notes can be changed.
 *
 * @author Jens Gulden
 */
public class MidiInsertControllersAction extends AbstractMidiAction {
    ...
    protected OptionsDialog createDialog(Component parent) {
        ...
        dialog.pack();
        dialog.setSize(dialog.getWidth() + 250, dialog.getHeight() + 50);
        return dialog;
    }
