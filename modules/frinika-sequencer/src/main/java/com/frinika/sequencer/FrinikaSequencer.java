 ...
 * skip notes / events on the first tick when starting to play - Able to insert
 * / remove events from the sequence while playing (not same as recording) -
 * Better abilities for song position monitoring NOTE: The Frinika sequencer is
 * not a complete implementation of the Java sequencer. Its primary goals is to
 * solve the needs of Frinika, and the implementation subset is thereafter.
 *
 * To be able to insert and remove events while playing the sequence has to be
 * reorganized from a vector, to a hashtable structure. In the default sequencer
 * pointers to the next midi event to be played is based on the vector index.
 * When inserting or removing notes before the current index the sequencer
 * pointer will break. By using a hashtable with keyword tick and value set to
 * an array of events occuring at that tick, one is able to lookup notes to be
 * played for a given tick position - thus not vulnerable to changes in the
 * ordering of events.
 *
 * The skipping of notes on the start tick, is caused by a bug in the binary
 * search routine of the default sequencer. This binary search is searching for
 * the first event to play after the given starttick. By using the mentioned
 * hashtable above, this is not an issue anymore.
 *
 * Issues with smooth looping is suspected though not verified, to be caused by
 * chasing of controller values when resetting the loop. The symptom is that the
 * longer out in the song you are, the loop gap is longer and longer.
 *
...
public class FrinikaSequencer implements Sequencer {
    ...
    private List<List<MultiEvent>> recordingTakes = new ArrayList<>();
    ...
    private List<MultiEvent> currentRecordingTake = new ArrayList<>();
    ...
    private Receiver receiver = new MonitorReceiver(getMidiMessageListeners(), new Receiver() // Jens
    {
        ...
        public void send(MidiMessage message, long timeStamp) {
            ...
            if (true) {
                return;  // try out new recording scheme
            }
            if (recording) {
                ...
                /**
                 *
                 * // The old recording code long tick =
                 * player.getRealTimeTickPosition(); for(Transmitter trans :
                 * transmitters) trans.getReceiver().send(message,-1);
                 * sequence.getFrinikaTrackWrappers().get(0).add( new
    ...
    final void newRecordingTake() {
        ...
        currentRecordingTake = new ArrayList<>();
    }
     ...
     * @return
     */
    public List<MultiEvent> getRecordingTake(int takeNo) {
    ...
    /**
     * Register a MidiOutDevice to this sequencer. It's also added to the list
     * of transmitters.
     *
     * @param midiDevice
    ...
    /**
     * Deregister a midi out device from this sequencer. Also removed from the
    ...
    //PJL
    /**
     * Add a tempo change listener to the sequencer. This is notified when we
     * play a tempo change event.
     *
     * @param listener
     */
    ...
    public MidiPlayOptions getPlayOptions(FrinikaTrackWrapper track) {
        ...
        if (opt == null) {
            return new MidiPlayOptions(); // if opt null, return a default one
        }
        return opt;
    }
    ...
    /**
     * Manually send a midi message using the channel/device settings of a
     * FrinikaTrackWrapper This is also used by wavexport
     *
     * @param msg
     * @param trackWrapper
    ...
    /**
     * If the player is not in realtime mode you can manually trigger the next
     * tick here
     *
     */
