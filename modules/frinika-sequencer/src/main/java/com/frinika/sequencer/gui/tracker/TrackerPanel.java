...
/**
 * A midi editor for editing a midi part in an old-school Amiga
 * Soundtracker/ProTracker fashion. For adapting to Midi there are differences
 * to the traditional tracker regarding handling of rows. First the row division
 * is not locked - but can be changed at any time - so that you can edit your
 * notes using the optimum "rows per beat" resolution. In traditional trackers
 * you were stuck to the originally chosen row resolution. Also if your notes is
 * between two rows - you can either increase the resolution - or you can use
 * the time column which shows how much your note is in between the rows (-0.5
 * to 0.5 - before or after).
 *
 * All note column-sets, consist of row-time (before or after the row), actual
 * note, velocity, and length of the note (number of rows). Notes are inserted
 * by pointing the cursor in the note column, and using the computer keyboard as
 * a piano with two octaves. Lower octave starting at Z making the twelve tone
 * octave like this: ZSXDCVGBHNJM - and the upper octave: Q2W3ER5T6Y7U.
 *
 * To insert controllers you also use a note column, but hold down CTRL while
 * typing a controller number. The velocity field is then used for controller
 * value. Pitch bend is CTRL-P in a note column and velocity of 0 is bend a note
 * down, 64 is no bend and 127 one note up.
 *
    ...
    void initComponents() {
        ...
        table = new JTable(tableModel) {
            ...
            public boolean isColumnSelected(int column) {
                ...
                if (columnModel.getSelectionModel().isSelectedIndex(c + 1)) {
                    return true;
                }
                if (columnModel.getSelectionModel().isSelectedIndex(c + 2)) {
                    return true;
                }
                if (columnModel.getSelectionModel().isSelectedIndex(c + 3)) {
                    return true;
                }
                if (columnModel.getSelectionModel().isSelectedIndex(c + 4)) {
                    return true;
                }
...
//    					column.setPreferredWidth(36);
            ...
            protected boolean processKeyBinding(KeyStroke ks, KeyEvent e,
                ...
                if (e.getKeyCode() == KeyEvent.VK_DELETE) {
                    ...
                    if (getSelectedRow() != -1) {
                        if (table.getSelectedColumn() > 0) //if(((table.getSelectedColumn() - 1)  % TrackerTableModel.COLUMNS) == TrackerTableModel.COLUMN_NOTEORCC) 
                        {
                            if (!pressed) {
                                return false;
                            }
                            ...
                            if (me != null) {
                                ...
                                {
                                    part.getEditHistoryContainer().mark(CurrentLocale.getMessage("sequencer.sysex.delete_sysex"));
                                } else {
                 ...
                 * Next similar column hotkeys feature Ctrl + Right / Ctrl +
                 * Left hotkeys to move the cursor one entire time/note/vel/len
                 * column right/left (i.e. to move 4 cols right/left; saves a
                 * few button hits) in tracker.
                 */
        ...
        controlPanel.add(new JLabel(
                CurrentLocale.getMessage("sequencer.tracker.automaticrowjump")));
        ...
        controlPanel.add(new JLabel(
                CurrentLocale.getMessage("sequencer.tracker.editvelocity")));
    ...
    /**
     * Update the selection container according to the selection of rows/columns
     * in the tracker.
     *
     */
    ...
    /**
     * Return how many rows to automatically jump when a note is hit (default is
     * 0)
     *
     * @return
     */
