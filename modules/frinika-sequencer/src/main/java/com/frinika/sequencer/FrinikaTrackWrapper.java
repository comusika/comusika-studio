...
/**
 * An extended version of the Track class in javax.sound.midi. The purpose of
 * the extension is to make the Sequencer player able to request all events for
 * a given tick, rather than having to perform a binary search, or keeping a
 * track index to get them.
 *
 * For the track mapping to remain consistent - it is important NOT to modify
 * the MidiEvent objects separately. If a MIDI event needs to be changed, it
 * should be removed and then added again to this Track.
 *
 * @author Peter Johan Salomonsen
 */
public class FrinikaTrackWrapper {

    private final HashMap<Long, Vector<MidiEvent>> tickMap = new HashMap<>();
    private final HashMap<Integer, SortedMap<Long, Integer>> controllerMap = new HashMap<>();
//    private final SortedMap<Long, MidiMessage> tempoMap = new TreeMap<>();

    private Track track;
    private Vector<MidiMessage> controllerStateMessages;
...
//        controllerStateMessages.ensureCapacity(128);
    ...
    /**
     *
     * clear this track of all events except the "META END OF TRACK"
     *
    ...
    /**
     * Add a MIDI event to the tickMap so that the TrackWrapper can keep track
     * on which events are located on what tick.
     *
     * @param event
     */
...
//	
    /**
     * Controller events will be added to the controlmap, so that the
     * trackwrapper can keep track of controller state at any point of time in
     * the sequence
     *
     * @param event
     */
    ...
    /**
     * Add a MidiEvent to the track - note that you should normally use
     * MultiEvents instead of MidiEvents
     *
     * @param event
     * @return
     */
    ...
    /**
     * Remove a MidiEvent from the track - Note that you should normally use
     * MultiEvents instead of MidiEvents
     *
     * @param event
     * @return
    ...
    public synchronized boolean remove(MidiEvent event) {
        if (track.remove(event)) {
            List<MidiEvent> tt = tickMap.get(event.getTick());
            if (tt == null) {
    ...
    /**
     * Used by the sequencer player to get the messages it should play at a
     * certain position
     *
     * @param tick
     * @return
     */
    public List<MidiEvent> getEventsForTick(long tick) {
        return tickMap.get(tick);
    }

    /**
     * Return a list of midimessages in order to restore controller states at a
     * specific tick. Used when looping a sequence, or when starting playback in
     * the middle of the song. This is a replacement for the chasing mechanism
     * in the JSE RealtimeSequencer implementation.
     *
     * @param tick
     * @return
     */
    ...
    public void attachToSequence() {
        ...
        if (sequence.frinikaTrackWrappers.contains(this)) {
            return;
        }
