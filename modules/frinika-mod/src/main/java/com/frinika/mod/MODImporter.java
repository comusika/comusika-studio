...
public class MODImporter {
    ...
    private void startPattern() {
        if (patternActive) {
            if (lastPatternSeperator == tickpos) {
                return;
            }
        }
        ...
        try {
            ...
            while (ok) {
                ...
                //if(tick == 0)
                {
                    ...
                    if (DEBUG_PRINT_PATTERN_WHILE_LOADING) {
                        if (tick == 0) {
                    ...
                    for (int i = 0; i < mixer.tracks.length; i++) {
                        ...
                        if (baserate == 0) {
                            ...
                            if (!mixer.tracks[i].newNoteEvent) {
                                if (DEBUG_PRINT_PATTERN_WHILE_LOADING) {
                                    if (tick == 0) {
                                        System.out.print(("  -- -" + ("-   ").substring(0, 2) + "" + (" p" + mixer.tracks[i].rate / baserate).substring(0, 5) + (" v" + mixer.tracks[i].volume).substring(0, 5) + "              ").substring(0, 18));
                                    }
                                }
                                ...
                                if (DEBUG_PRINT_PATTERN_WHILE_LOADING) {
                                    if (tick == 0) {
                                        System.out.print(("  " + ts[i].getNote() + ":i" + (ts[i].getInstrument() + "  ").substring(0, 2) + "" + (" p" + mixer.tracks[i].rate / baserate).substring(0, 5) + (" v" + mixer.tracks[i].volume).substring(0, 5) + "           ").substring(0, 18));
                                    }
                                }
